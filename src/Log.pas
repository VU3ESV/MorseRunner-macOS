...
uses
  SysUtils, Classes, Graphics, RndFunc, Math;
...
begin
  ...
  // (port) header row is plain text; the original underlined/colored it via
  // RichEdit SelAttributes, which the LCL TMemo used here does not support.
...
begin
  with MainForm do
    ...
    if (Length(Edit1.Text) < 3) or (Length(Edit2.Text) <> 3) or (Edit3.Text = '')
      then begin {(port) LCL has no cross-platform Beep} Exit; end;
...
begin
  ...
  // (port) scroll to the newest line. The original also colored the Chk column
  // red via RichEdit SelAttributes; the LCL TMemo used here keeps the text but
  // not the color.
